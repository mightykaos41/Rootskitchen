...
export default {
  async fetch(request, env) {
    ...
    /* ---- cookbook email list (stored in KV, exported via wrangler) ---- */
    if (url.pathname === "/api/subscribe" && request.method === "POST") {
      if (!env.QUOTA) return json({ error: "Storage not configured" }, 503);
      let body;
      try { body = await request.json(); } catch { return json({ error: "Invalid JSON" }, 400); }
      ...
      /* key-per-email = automatic dedupe; no TTL = kept until you delete it */
      await env.QUOTA.put("email:" + email, JSON.stringify({ joined: new Date().toISOString() }));
      return json({ ok: true });
    }
